"""
From github.com/shntnu/pycytominer/blob/e7ef835daac20813bb92e4bad28b0e79a60d368b/
...
References
----------
.. [1] Kessy et al. 2016 "Optimal Whitening and Decorrelation"
arXiv: https://arxiv.org/abs/1512.00809
"""
...
class Spherize(BaseEstimator, TransformerMixin):
    ...
    3) Notes from Niranj Chandrasekaran
    (https://github.com/cytomining/pycytominer/issues/90)
    4) The R package "whitening" written by Strimmer et al
    (http://strimmerlab.org/software/whitening/)
    ...
    def fit(self, X, y=None):
        ...
        # If n < d, then rank should be equal to n - 1 (if centered)
        # or n (if not centered)
        ...
        if n <= d:
            ...
            if Sigma.shape[0] != n:
                error_msg = f"When n <= d, Sigma should have shape (n,) i.e. ({n}, ) but it is {Sigma.shape}. the call to `np.linalg.svd` in `pycytominer.transform.Spherize`"  # NOQA
